...
def quantum_sign_estimate(Q: np.ndarray, V_T: Callable, dev: qml.device):
    # QとV_Tの位相の差を測定する量子回路を構築
    # ...
    # 測定結果から符号を計算して返す
    # return sign
    pass
...
def quantum_afqmc(
    ...
    inputs = [
        (num_steps, quantum_evaluations_every_n_steps, dtau, trial, prop, Ehf, walker, weight, dev, V_T)
...
def q_full_imag_time_evolution(
    ...
    weight: float,
    dev: qml.device,
    V_T: Callable, #qcqmcにするため、初期状態を新たに追加する
):
    ...
    for time in range(num_steps):
        ...
        E_loc, num, denom, walker, weight = imag_time_propogator_qaee(
                dtau, trial, walker, weight, prop, E_shift, dev, V_T
        )
...
def imag_time_propogator_qaee(
    ...
    ovlp = amplitude_estimate(walker, V_T, dev)
    ...
    new_walker = q_propogate_walker(x, prop.v_0, prop.v_gamma, prop.mf_shift, dtau, walker, V_T, q_ovlp, dev) #q_ovlpを渡し、V_Tを変数として渡そう 
    ...
    arg = np.angle(new_ovlp / q_ovlp)
    new_weight = weight * np.exp(-dtau * (np.real(E_loc_q) - E_shift)) * np.max([0.0, np.cos(arg)])
    # new_weight = weight * np.exp(-dtau * (np.real(E_loc) - E_shift)) * ovlp

    # numerator = E_loc_q / q_ovlp
    # denominator = q_ovlp / q_ovlp 
